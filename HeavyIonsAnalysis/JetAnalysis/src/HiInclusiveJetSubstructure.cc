...
void HiInclusiveJetSubstructure::beginJob() {
 ...
 // t->Branch("jtangu",jets_.jtangu,"jtangu[nref]/F");
  ...
  if(isMC_){
    ...
    //t->Branch("ref_z",jets_.ref_z,"ref_z[nref]/F");
    //t->Branch("ref_rg",jets_.ref_rg,"ref_rg[nref]/F");
    //t->Branch("ref_ktg",jets_.ref_ktg,"ref_ktg[nref]/F");
...
void HiInclusiveJetSubstructure::analyze(const Event& iEvent, const EventSetup& iSetup) 
{
  ...
  //Ho appena decommentato questi due print
  ...
  for(unsigned int j = 0; j < jets->size(); ++j){
    ...
    //std::cout << "( # jtJetConstituent, # refJetConstituent ) = ( " << jets_.jtJetConstituent.size() << ", " << jets_.refJetConstituent.size() << " ) sizes of consts" << std::endl;
    ...
    if(isMC_){
      ...
      jets_.refphi[jets_.nref] = genjet->phi();
          ...
          //std::cout<<"jet daughters gen"<<genjet->numberOfDaughters()<<std::endl;
...
void HiInclusiveJetSubstructure::IterativeDeclusteringRec(double groom_type, double groom_combine, const reco::Jet& jet, fastjet::PseudoJet *sub1, fastjet::PseudoJet *sub2)
{
  ...
  double dyn_deltaR = 0;
  double flagtag;// io
  double flagprompt = 0; //io, per seguire il D
  double flagprod = 0; //io, per seguire il D
  ...
  try{
    ...
    for(auto it = daughters.begin(); it!=daughters.end(); ++it){
      ...
      //if((**it).mass() >1 ) cout<<"There is a D"<<endl; 

      //Taglio sul pt delle subparticles
      if((**it).pt()<1) continue;
    ...
    if(!jj.has_parents(j1,j2)){
      ...
      { //Qui e' la condizione che deve soddisfare per essere inclusa con l algo softdrop. Nota che qui si entra solo se la flag del SoftDrop =0. poi viene aggiornata a =1 all'interno di questa scope, cosi prendo solamente il primo angolo che soddisfa la condizione di SD.
        ...
        ktg_SD = k_t;
        //flagl=flagtag;
        //flagprod=flagprompt; // io, per seguire il D
      }
    ...
    //jets_.jtdyn_z[jets_.nref] = dyn_z;
        //jets_.jt_z[jets_.nref] = zg;
        //jets_.jt_rg[jets_.nref] = rg;
        //jets_.jt_ktg[jets_.nref] =ktg;
...
void HiInclusiveJetSubstructure::IterativeDeclusteringGen(double groom_type, double groom_combine,const reco::GenJet& jet,fastjet::PseudoJet *sub1,fastjet::PseudoJet *sub2)
{
  ...
  try{
    ...
    if(!jj.has_parents(j1,j2)){
      ...
      if( ( (groom_combine == 0) && (groom_type == 1) && (z > mysdcut1) && (sdFlag == 0) ) || ( (groom_combine == 1) && (z > mysdcut1) && (sdFlag == 0) ) ){ //Qui e' la condizione che deve soddisfare per essere inclusa con l algo softdrop. Nota che qui si entra solo se la flag del SoftDrop =0. poi viene aggiornata a =1 all'interno di questa scope, cosi prendo solamente il primo angolo che soddisfa la condizione di SD.
        sdFlag=1;
        //zg = z;
        //rg  = delta_R;
        //ktg=k_t;
      ...
      if( ( (groom_combine == 0 ) && (groom_type == 0) && (k_t > 1) ) || ( (groom_combine == 1 ) && (k_t > 1) ) ){	//late kt    
        //zg=z;
        //rg=delta_R;
        //ktg = k_t;
        zg_latekt = z;
  ...
  nsplit = nsplit+1;
	...
	//jets_.ref_z[jets_.nref] = zg;
	//jets_.ref_rg[jets_.nref] = rg;
	//jets_.ref_ktg[jets_.nref] =ktg;
...
void HiInclusiveJetSubstructure::RecoTruthSplitMatching(std::vector<fastjet::PseudoJet> &constituents_level1, fastjet::PseudoJet &hardest_level2, bool *bool_array, int *hardest_level1_split){
    ...
    //loop over constituents of reco jet
